...
try:
    ...
    print("[ERROR] Could not load LSTM model:", e)
    model = None
labels = ['forward','stop']
    ...
    while True:
        with data_lock:
            if len(raw_vals) >= 1024:
                # Ambil 1024 terakhir
                segment = np.array(raw_vals[-1024:], dtype=np.float32)
                # Ubah ke bentuk (128, 8)
                segment = segment.reshape((128, 8))
                # Tambah batch dimension -> (1, 128, 8)
        ...
        if segment is not None and model is not None:
            try:
                pred_probs = model.predict(segment, verbose=0)
